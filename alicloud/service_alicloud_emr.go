...
func (s *EmrService) GetEmrClusterNew(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewEmrClient()
	if err != nil {
		return nil, WrapError(err)
...
func (s *EmrService) WaitForEmrClusterNew(id string, status Status, timeout int) error {
	deadline := time.Now().Add(time.Duration(timeout) * time.Second)
	for {
		object, err := s.GetEmrClusterNew(id)
		if err != nil {
			if NotFoundError(err) {
				if status == Deleted {
					return nil
...
func (s *EmrService) EmrClusterNewStateRefreshFunc(id string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.GetEmrClusterNew(id)
		if err != nil {
			if NotFoundError(err) {
